...
export async function POST(req: NextRequest) {
  ...
  if (!body?.events) return NextResponse.json({ error: 'No events provided' }, { status: 400 })
  ...
  try {
    const content = JSON.stringify(body.events, null, 2) + '\n'
    const sha = await getFileSha(ghPat, owner, repo, EVENTS_PATH)
    await commitFile(ghPat, owner, repo, EVENTS_PATH, content, sha)
  } catch (err) {
